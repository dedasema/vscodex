...
export class CodexVSProvider implements vscode.LanguageModelChatProvider {
  ...
  constructor(
    ...
    this.disposables.push(
      ...
      vscode.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration('codexvs')) {
    ...
    try {
      ...
      if (error instanceof AppServerProtocolError) {
        await vscode.window.showErrorMessage(
          `Codex model discovery is incompatible with the installed app-server (${error.message}) Check the CodexVS logs or update Codex CLI.`
        );
        return [];
      }
      await vscode.window.showErrorMessage(
...
async function promptForChatGptSignIn(message: string): Promise<boolean> {
  ...
  if (action === 'Sign in with ChatGPT') {
    await vscode.commands.executeCommand('codexvs.signInWithChatGPT');
    return true;
  }
  if (action === 'Use Device Code') {
    await vscode.commands.executeCommand('codexvs.signInWithDeviceCode');
    return true;
  }
  return false;
}
